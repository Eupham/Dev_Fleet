...
def build_llama_image(repo_id: str, filename: str, **kwargs) -> modal.Image:
    ...
    return (
        ...
        .env({
            ...
            # REMOVED LD_LIBRARY_PATH from here so it doesn't persist to runtime
        }) 
        .pip_install("huggingface_hub", "langgraph>=1.1.2", "mcp>=1.26.0")
        
        # FIX: Export the stub path strictly for this exact compilation step
        .run_commands(
            "export LD_LIBRARY_PATH=/usr/local/cuda/lib64/stubs && "
            "pip install git+https://github.com/abetlen/llama-cpp-python.git --upgrade --no-cache-dir --force-reinstall"
        )
...
class BaseInference:
    def start_logic(self, cfg: dict):
        ...
        # --- DIAGNOSTIC CHECK --- (Keep this, it's great)
        if os.path.exists(model_path):
            size_mb = os.path.getsize(model_path) / (1024 * 1024)
            if size_mb < 1.0:
                raise RuntimeError(f"CRITICAL: Downloaded a pointer file. Size: {size_mb} MB.")
        else:
        ...
        self.llm = Llama(
            ...
            n_gpu_layers=-1, 
            n_ctx=cfg["n_ctx"],
            verbose=True,    # <--- FIX: Turn ON to see internal C++ errors
            use_mmap=True,   # <--- FIX: Turn back ON to prevent silent OOM crashes
        )
