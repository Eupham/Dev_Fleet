...
_small_image = (
    ...
    .run_commands(
        [f"python -c \"from huggingface_hub import snapshot_download; snapshot_download('{_SMALL_MODEL}')\"", f"python -m vllm.entrypoints.openai.api_server --model {_SMALL_MODEL} --version || true"],
...
_medium_image = (
    ...
    .uv_pip_install("vllm==0.17.1", "hf_transfer")
    .run_commands(
        [f"python -c \"from huggingface_hub import snapshot_download; snapshot_download('{_MEDIUM_MODEL}')\"", f"python -m vllm.entrypoints.openai.api_server --model {_MEDIUM_MODEL} --version || true"],
        env={"HF_HUB_ENABLE_HF_TRANSFER": "1"},
    )
    .env({
        "VLLM_SERVER_DEV_MODE": "1",
...
_large_image = (
    ...
    .uv_pip_install("vllm==0.17.1", "hf_transfer")
    .run_commands(
        [f"python -c \"from huggingface_hub import snapshot_download; snapshot_download('{_LARGE_MODEL}')\"", f"python -m vllm.entrypoints.openai.api_server --model {_LARGE_MODEL} --version || true"],
        env={"HF_HUB_ENABLE_HF_TRANSFER": "1"},
    )
    .env({
        "VLLM_SERVER_DEV_MODE": "1",
